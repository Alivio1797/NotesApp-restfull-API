...
class NotesApp {
  ...
  async toggleArchiveView() {
    try {
      ...
      showAlert.error('Failed to toggle notes view');
    } finally {
      this.hideLoading();
    }
  }
  ...
  async renderNotes() {
    try {
      ...
      showAlert.error('Failed to load notes. Please try again later.');
    }
  }
  ...
  async renderArchivedNotes() {
    try {
      ...
      showAlert.error('Failed to load archived notes. Please try again later.');
    }
  }
  ...
  async handleArchiveNote(id) {
    try {
      ...
      if (isSuccess) {
        await this.renderNotes();
        showAlert.success('Note archived successfully!');
      } else {
        throw new Error('Archive operation failed');
      }
    } catch (error) {
      console.error('Error archiving note:', error);
      showAlert.error('Failed to archive note. Please try again.');
    } finally {
      this.hideLoading();
    }
  }
  ...
  async handleUnarchiveNote(id) {
    try {
      ...
      if (isSuccess) {
        await this.renderArchivedNotes();
        showAlert.success('Note unarchived successfully!');
      } else {
        throw new Error('Unarchive operation failed');
      }
    } catch (error) {
      console.error('Error unarchiving note:', error);
      showAlert.error('Failed to unarchive note. Please try again.');
    } finally {
      this.hideLoading();
    }
  }
